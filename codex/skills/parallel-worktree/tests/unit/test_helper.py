...
class HelperTest(unittest.TestCase):
    ...
    def test_approved_ignored_manifest_allows_benign_file_and_rejects_secret(self) -> None:
        (self.repo / ".gitignore").write_text("cache.txt\n")
        git(self.repo, "add", ".gitignore")
        git(self.repo, "commit", "-m", "test: ignore cache")
        git(self.repo, "push", "origin", "main")

        record = self.helper_json(
            "prepare-start", "--repo", str(self.repo), "--issue", "143",
            "--risk", "R1", "--planned-branch", "fix/143-example",
        )
        operation = record["operation_id"]
        registry_dir = self.codex_home / "parallel-worktree" / record["repository_id"]
        approvals = registry_dir / "approvals"
        approvals.mkdir(mode=0o700)
        approval = approvals / "ignored-files.json"
        content = b"generated cache\n"
        ...
        approved = self.helper_json(
            "approve-ignored-manifest", "--repo", str(self.repo), "--issue", "143",
            "--operation", operation, "--approval-file", str(approval),
        )
        self.assertEqual(approved["approved_ignored_files"][0]["path"], "cache.txt")

        added = self.helper_json(
            "worktree-add", "--repo", str(self.repo), "--issue", "143", "--operation", operation,
        )
        late_approval = run(
            "approve-ignored-manifest", "--repo", str(self.repo), "--issue", "143",
            "--operation", operation, "--approval-file", str(approval), env=self.env, check=False,
        )
        self.assertNotEqual(late_approval.returncode, 0)
        self.assertIn("before worktree provisioning", late_approval.stderr)
        (Path(added["worktree_path"]) / "cache.txt").write_bytes(content)
        verified = self.helper_json(
            "verify-child", "--repo", str(self.repo), "--issue", "143", "--operation", operation,
        )
        self.assertEqual(verified["provisioning_phase"], "verified_clean")

        secret_record = self.helper_json(
            "prepare-start", "--repo", str(self.repo), "--issue", "144",
            "--risk", "R1", "--planned-branch", "fix/144-example",
        )
        secret_approval = approvals / "ignored-secret.json"
        secret_approval.write_text(json.dumps({
            "user_approved": True,
            "files": [{
                "path": ".env.local",
                "type": "file",
        ...
        secret_approval.chmod(0o600)
        rejected = run(
            "approve-ignored-manifest", "--repo", str(self.repo), "--issue", "144",
            "--operation", secret_record["operation_id"],
            "--approval-file", str(secret_approval), env=self.env, check=False,
        )
        self.assertNotEqual(rejected.returncode, 0)
    ...
    def test_cleanup_requires_verified_evidence_and_two_phase_authorization(self) -> None:
        ...
        removed_status = self.helper_json(
            "status", "--repo", str(self.repo), "--issue", "133",
            "--task-evidence-file", str(evidence),
        )
        self.assertEqual(removed_status["suggested_state"], "cleanup_pending")
        expired = json.loads(registry.read_text())
        expired["lease_expires_at"] = "2000-01-01T00:00:00Z"
        registry.write_text(json.dumps(expired))
        ...
        archived_status = self.helper_json(
            "status", "--repo", str(self.repo), "--issue", "133",
            "--task-evidence-file", str(evidence),
        )
        self.assertEqual(archived_status["suggested_state"], "archived")
