...
7. If ignored/setup files are needed, record the explicitly approved path/type/size/hash manifest with `pw-helper approve-ignored-manifest` before creating the worktree.
8. Release the repository lock, then the Issue lock before any UI, task, or user wait.
...
- A missing worktree is normal after authorized removal in `cleanup_pending` and after finalization in `archived`; archived task inventory must be empty.
...
Accept only R0-R4 values classified under the current `git-workflow` development policy. That policy owns model, implementation, authorization, and final-review routing; do not duplicate its table here.
