...
4. Give the reviewer only acceptance criteria, repository/base/branch, the complete frozen-diff scope, exact fingerprint, and checks already run. Do not include implementation conclusions or suspected safe areas.
...
10. Restage the complete intended scope and freeze a new fingerprint once after the fix batch. Send the complete diff to the same saved agent as `Round N`. Repeat until no P0-P2 remains; do not re-review solely for deferred P3.
